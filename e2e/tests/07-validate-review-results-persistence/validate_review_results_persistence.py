...
class ValidateReviewResultsPersistenceTest:
    ...
    def _is_feedback_valid(self, feedback: str) -> bool:
        """Check if feedback string is valid (not empty or whitespace).
    ...
    def _get_missing_roles(
        ...
        """Get list of roles with missing feedback.

        Args:
            architect_feedback: Architect feedback string
            qa_feedback: QA feedback string
            devops_feedback: DevOps feedback string

        Returns:
            List of role names with missing feedback
        """
        missing_roles = []
        if not self._is_feedback_valid(architect_feedback):
            missing_roles.append("ARCHITECT")
        if not self._is_feedback_valid(qa_feedback):
            missing_roles.append("QA")
        if not self._is_feedback_valid(devops_feedback):
            missing_roles.append("DEVOPS")
        return missing_roles

    def _format_role_status(self, feedback: str) -> str:
        ...
        Returns:
            '✓' if valid, '✗' otherwise
        """
        return "✓" if self._is_feedback_valid(feedback) else "✗"

    def _verify_single_review_result(self, review_result) -> tuple[bool, str]:
        """Verify a single review result has all required roles.

        Args:
            review_result: Review result protobuf object

        Returns:
            Tuple of (is_valid, story_id)
        """
        ...
        missing_roles = self._get_missing_roles(
            architect_feedback, qa_feedback, devops_feedback
        )

        print_info(
            f"Story {story_id}: "
            f"ARCHITECT={self._format_role_status(architect_feedback)}, "
            f"QA={self._format_role_status(qa_feedback)}, "
            f"DEVOPS={self._format_role_status(devops_feedback)}"
        )
        ...
        if missing_roles:
            ...
            return False, story_id

        return True, story_id

    async def _get_review_results(self) -> Optional[list]:
        """Get review results from ceremony.

        Returns:
            List of review results if successful, None otherwise
        """
        if not self.planning_stub or not self.ceremony_id:
        ...
        try:
            ...
            return response.ceremony.review_results

        except grpc.RpcError as e:
            print_error(f"gRPC error: {e.code()} - {e.details()}")
            return None
        except Exception as e:
            print_error(f"Error getting review results: {e}")
            return None

    def _verify_all_review_results(self, review_results: list) -> bool:
        """Verify all review results have complete feedback from all roles.

        Args:
            review_results: List of review result protobuf objects

        Returns:
            True if all review results are complete, False otherwise
        """
        print_info(f"Verifying review results for {len(review_results)} stories...")

        for review_result in review_results:
            is_valid, _ = self._verify_single_review_result(review_result)
            if not is_valid:
                return False
    ...
    async def verify_review_results_complete(self) -> bool:
        ...
        """
        review_results = await self._get_review_results()
        if review_results is None:
            return False

        return self._verify_all_review_results(review_results)
