#!/usr/bin/env python3
"""E2E test: repo changed files + symbol intelligence tools (CAT-015)."""
...
class WorkspaceRepoSymbolIntelligenceE2E:
    def __init__(self) -> None:
        self.workspace_url = os.getenv(
            "WORKSPACE_URL",
            "http://workspace.swe-ai-fleet.svc.cluster.local:50053",
        ).rstrip("/")
        self.evidence_file = os.getenv("EVIDENCE_FILE", f"/tmp/e2e-32-{int(time.time())}.json")
        self.run_id = f"e2e-ws-repo-symbol-intel-{int(time.time())}"
        self.sessions: list[str] = []
        self.invocation_counter = 0

        self.evidence: dict[str, Any] = {
            "test_id": "32-workspace-repo-symbol-intelligence",
            "run_id": self.run_id,
            "status": "running",
    ...
    def _create_session(self) -> str:
        payload = {
            "principal": {
                ...
                "actor_id": "e2e-repo-symbol-intelligence",
                "roles": ["developer"],
            },
            "allowed_paths": ["."],
    ...
    def run(self) -> int:
        ...
        try:
            ...
            print_success("Catalog exposes CAT-015 tools")

            print_step(3, "Create code sample")
            source_code = "\n".join(
                [
                    "package todo",
                    "",
                    ...
                    "}",
                    "",
                    "func ProcessTodo() Todo {",
                    "    created := CreateTodo(\"demo\")",
                    "    return created",
                    "}",
                ]
            )
            ...
            self._record_step("seed_code", "passed")
            print_success("Sample code written")

            print_step(4, "Invoke repo.changed_files")
            synthetic_status = "\n".join(
                [
                    " M internal/todo/service.go",
            ...
            _, body, inv = self._invoke(
                ...
                args={"output": synthetic_status, "path": ".", "max_files": 50, "include_untracked": True},
            )
            self._assert_succeeded(invocation=inv, body=body, label="repo.changed_files")
            changed_output = inv.get("output", {}) if isinstance(inv, dict) else {}
            changed_count = int(changed_output.get("changed_count", 0))
            if changed_count < 2:
                raise RuntimeError(f"repo.changed_files changed_count too low: {changed_output}")
            invocation_id = str(inv.get("id", "")).strip() if isinstance(inv, dict) else ""
            self._assert_artifact_present(invocation_id, "changed-files-summary.json")
            self._record_step("repo_changed_files", "passed", {"changed_count": changed_count})
            print_success("repo.changed_files succeeded")

            print_step(5, "Invoke repo.symbol_search")
            _, body, inv = self._invoke(
                session_id=session_id,
                tool_name="repo.symbol_search",
                args={"symbol": "CreateTodo", "path": "internal", "whole_word": True, "max_results": 20},
            )
            self._assert_succeeded(invocation=inv, body=body, label="repo.symbol_search")
            search_output = inv.get("output", {}) if isinstance(inv, dict) else {}
            matches_count = int(search_output.get("matches_count", 0))
            if matches_count < 2:
                raise RuntimeError(f"repo.symbol_search matches_count too low: {search_output}")
            invocation_id = str(inv.get("id", "")).strip() if isinstance(inv, dict) else ""
            self._assert_artifact_present(invocation_id, "symbol-search-results.json")
            self._record_step("repo_symbol_search", "passed", {"matches_count": matches_count})
            print_success("repo.symbol_search succeeded")

            print_step(6, "Invoke repo.find_references")
            _, body, inv = self._invoke(
                session_id=session_id,
                tool_name="repo.find_references",
                args={"symbol": "CreateTodo", "path": "internal", "include_declarations": False, "max_references": 20},
            )
            self._assert_succeeded(invocation=inv, body=body, label="repo.find_references")
            references_output = inv.get("output", {}) if isinstance(inv, dict) else {}
            references_count = int(references_output.get("references_count", 0))
            if references_count < 1:
                raise RuntimeError(f"repo.find_references references_count too low: {references_output}")
            references = references_output.get("references", [])
            if isinstance(references, list):
                for item in references:
                    if isinstance(item, dict) and bool(item.get("is_declaration", False)):
                        raise RuntimeError(f"repo.find_references returned declaration while excluded: {item}")
            invocation_id = str(inv.get("id", "")).strip() if isinstance(inv, dict) else ""
            self._assert_artifact_present(invocation_id, "references-summary.json")
            self._record_step("repo_find_references", "passed", {"references_count": references_count})
            print_success("repo.find_references succeeded")

            final_status = "passed"
            self._record_step("test_result", "passed")
            print_success("E2E 32 completed successfully")
            return 0
        except Exception as exc:
...
def main() -> int:
    return WorkspaceRepoSymbolIntelligenceE2E().run()
