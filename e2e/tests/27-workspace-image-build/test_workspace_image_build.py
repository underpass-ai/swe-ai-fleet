#!/usr/bin/env python3
"""E2E test: image.build tool.

Validates:
- catalog exposes image.build
- image.build enforces approval
- image.build returns deterministic structured output
- build report artifact is generated
"""
...
class WorkspaceImageBuildE2E:
    def __init__(self) -> None:
        self.workspace_url = os.getenv(
            "WORKSPACE_URL",
            "http://workspace.swe-ai-fleet.svc.cluster.local:50053",
        ).rstrip("/")
        self.evidence_file = os.getenv("EVIDENCE_FILE", f"/tmp/e2e-27-{int(time.time())}.json")
        self.run_id = f"e2e-ws-image-build-{int(time.time())}"
        self.sessions: list[str] = []
        self.invocation_counter = 0

        self.evidence: dict[str, Any] = {
            "test_id": "27-workspace-image-build",
            "run_id": self.run_id,
            "status": "running",
    ...
    def run(self) -> int:
        ...
        try:
            ...
            if "image.build" not in tools:
                raise RuntimeError("catalog missing image.build")
            self._record_step("catalog", "passed", {"tool_count": len(tools)})
            print_success("Catalog exposes image.build")
            ...
            print_step(4, "Validate approval required")
            status, body, inv = self._invoke(
                session_id=session_id,
                tool_name="image.build",
                args={"context_path": ".", "dockerfile_path": "Dockerfile", "tag": "ghcr.io/acme/e2e-build:latest"},
                approved=False,
            )
            if status != 428:
                raise RuntimeError(f"expected 428 without approval, got {status}: {body}")
            self._record_step("approval_required", "passed")
            print_success("image.build requires explicit approval")

            print_step(5, "Invoke image.build")
            _, body, inv = self._invoke(
                session_id=session_id,
                tool_name="image.build",
                args={
                    ...
                    "tag": "ghcr.io/acme/e2e-build:latest",
                    "push": False,
                },
                approved=True,
            )
            self._assert_succeeded(invocation=inv, body=body, label="image.build")
            ...
            if not digest.startswith("sha256:"):
                raise RuntimeError(f"expected digest sha256:..., got {digest!r}")
            image_ref = str(output.get("image_ref", "")).strip()
            if "@sha256:" not in image_ref:
                raise RuntimeError(f"expected digest-pinned image_ref, got {image_ref!r}")
            issues_count = int(output.get("issues_count", 0))
            if issues_count < 0:
                raise RuntimeError(f"invalid issues_count: {issues_count}")
            invocation_id = str(inv.get("id", "")).strip() if isinstance(inv, dict) else ""
            if not invocation_id:
                raise RuntimeError("image.build missing invocation id")
            self._assert_artifact_present(invocation_id, "image-build-report.json")
            ...
            print_success("image.build succeeded with structured output")

            final_status = "passed"
            self._record_step("test_result", "passed")
            print_success("E2E 27 completed successfully")
            return 0
