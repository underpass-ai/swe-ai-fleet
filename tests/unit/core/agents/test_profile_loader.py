...
from core.agents.profile_loader import (
    AgentProfile,
    ROLE_MODEL_MAPPING,
...
""")
    ...
    def test_get_profile_for_role_yaml_load_error(self):
        """Test YAML loading error falls back to defaults."""
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("core.agents.profile_loader.logger") as mock_logger:
                ...
                assert profile == ROLE_MODEL_MAPPING["ARCHITECT"]
...
""")
    ...
    def test_role_model_mapping_completeness(self):
        """Test ROLE_MODEL_MAPPING contains all expected roles."""
        expected_roles = {"ARCHITECT", "DEV", "QA", "DEVOPS", "DATA"}
        
        assert set(ROLE_MODEL_MAPPING.keys()) == expected_roles

    def test_role_model_mapping_all_have_required_fields(self):
        """Test all role mappings have required fields."""
        required_fields = {"model", "temperature", "max_tokens", "context_window"}
        
        for role, mapping in ROLE_MODEL_MAPPING.items():
            assert set(mapping.keys()) == required_fields
            assert isinstance(mapping["model"], str)
            assert isinstance(mapping["temperature"], (int, float))
            assert isinstance(mapping["max_tokens"], int)
            assert isinstance(mapping["context_window"], int)
    ...
    def test_get_profile_for_role_returns_dict_with_required_keys(self):
        ...
        for role in ROLE_MODEL_MAPPING.keys():
