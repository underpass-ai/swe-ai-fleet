...
async def test_001_po_creates_story_validates_persistence(
    ...
    try:
        ...
        print(f"\n📊 Data preserved for inspection - Story ID: {story_id}")

    finally:
        # ========== CLEANUP DISABLED - DATA PRESERVED FOR INSPECTION ==========
        # Note: Data is intentionally preserved to allow manual inspection
        # Use clear-test-data.sh to clean up when needed
        print(f"📊 Data preserved for inspection: {story_id}")
        print("   View with: ./view-test-data.sh")
        print("   Clean with: ./clear-test-data.sh")
...
async def test_001b_story_creation_validates_phase_transition(
    ...
    try:
        # ========== ACT ==========
        # Step 1: Create story
        await context_client.create_story(
            story_id=story_request.story_id,
            title=story_request.title,
            description=story_request.description,
            initial_phase=story_request.initial_phase
