"""
NATS JetStream Streams Initialization for Orchestrator Service.

Creates the necessary streams if they don't exist.
This module is specific to the Orchestrator service and should not be shared
with other services to maintain architectural boundaries.
"""
...
async def ensure_streams(js: "JetStreamContext") -> None:
    """
    Ensure all required NATS JetStream streams exist for Orchestrator.

    Args:
        js: JetStream context
    """
    # Lazy import to avoid circular import issues
    from nats.js import JetStreamContext as _JetStreamContext
    ...
    for stream_def in streams:
        try:
            ...
            logger.info(
                f"✓ Created PERSISTENT stream: {stream_name} (storage: FILE)"
            )
        except Exception as e:
            # Log warning but continue - streams might already exist
            logger.warning(f"Stream {stream_name}: {e}")

    logger.info("✓ All persistent streams ensured for Orchestrator")
