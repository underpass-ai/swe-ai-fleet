...
class TestOrchestratorPlanningConsumer:
    ...
    async def test_planning_consumer_start_creates_subscriptions(self):
        ...
        consumer._poll_story_transitions = AsyncMock()  # type: ignore[attr-defined]
        consumer._poll_plan_approvals = AsyncMock()  # type: ignore[attr-defined]

        # Call start(), which will create background polling tasks.
        # We don't patch asyncio.create_task here to avoid un-awaited coroutine warnings
        # and to keep the test focused on subscription creation via the MessagingPort.
