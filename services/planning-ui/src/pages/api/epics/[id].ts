...
import { buildGetEpicRequest } from '../../../lib/grpc-request-builders';
...
export const GET: APIRoute = async ({ params }) => {
  try {
    ...
    const requestPayload = buildGetEpicRequest({
      epic_id: id,
    });

    const response = await promisifyGrpcCall(
      (req, callback) => client.getEpic(req, callback),
      requestPayload
    );
    ...
    if (!response.success || !response.epic) {
      return new Response(
        JSON.stringify({
          success: false,
          message: response.message || 'Epic not found',
        }),
        {
          status: 404,
          headers: { 'Content-Type': 'application/json' },
    ...
    return new Response(
      JSON.stringify({
        epic: response.epic,
        success: true,
        message: response.message || 'Epic retrieved successfully',
      }),
      {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
